  ...
  return (
    <section className="relative overflow-hidden py-16 md:py-24">
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute -top-32 left-1/4 h-[480px] w-[480px] rounded-full bg-[#0079CB]/25 blur-[130px]" />
      ...
      <div className="container-lp">
        ...
        >
          {eyebrow && (
            <motion.span variants={vEyebrow} className="eyebrow !text-[#0ed8f6]">
              {eyebrow}
            </motion.span>
          )}
          <motion.h1
          ...
          >
            {title}
            {highlight && <span className="text-[#0ed8f6]"> {highlight}</span>}
          </motion.h1>
          {description && (
            <motion.div
              variants={vSubtitle}
              className="mt-5 space-y-4 text-lg leading-relaxed text-ink-muted"
            >
              {description}
            </motion.div>
          )}
